    ...
    for stage in stages:
        ...
        for segment in segments:
            if segment == "全体":
                ...
                if stage == "友だち数":
                    value = sum(
                        target_df[f"{seg}友だち数"].iloc[0] for seg in ["新規", "既存"]
                    )
                else:
                    value = sum(
                        target_df[f"{seg}{stage}"].iloc[0] for seg in ["新規", "既存"]
                    )
        ...
        result_data.append({"stage": stage, **stage_data})
...
def create_funnel_chart(segment_data, segment_type):
    ...
    y_labels = [
        f"{stage} (計: {int(sum(segment_data[segment_data['stage'] == stage][segment].iloc[0] for segment in segments)):,}人)"
...
def create_segment_funnel_data(df, segment_type, target_date):
    ...
    for segment in segments:
        ...
        for stage in stages:
            ...
            segment_values.append({"stage": stage, "value": value, "cvr": cvr})
